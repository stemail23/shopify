...
function toShopifyEntityArray(csvData, callback) {
	...
	csvParse(csvData, options, shopifyEntityArrayFromCsvEntityArrayCallback(function(err, result) {
		if (err) { return callback(err); }
		if (result && result[0] && result[0].id === 'id') { result = result.slice(1, result.length); }
